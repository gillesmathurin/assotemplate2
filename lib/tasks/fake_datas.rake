...
task :load_fakedatas => :environment do
  ...
  @association = Association.create(:nom => "Agouti", :adresse => "rue Lethière", :email => Faker::Internet.email,
   ...
   :ville => Faker::Address.city, :tel => Faker::Address.phone_number, :fax => Faker::Address.phone_number,
   :about_us => Faker::Lorem.paragraph, :date_creation => "1980-05-24")
  
  # Creation des Users-Membres
  1.upto(4) do |x|
    User.create(:login => Faker::Internet.user_name, :email => Faker::Internet.email, :name => Faker::Name.name, :password => "secret#{x}",
  ...
  User.create(:login => "beta", :email => Faker::Internet.email, :name => Faker::Name.name, :password => "betapass",
    :password_confirmation => "betapass", :usertype => "1", :state => 'active')
  
  # Creation de 3 articles
  1.upto(3) do
    @user = User.find(:first, :offset => (Player.count * rand).to_i)
  ...
  1.upto(3) do
    Evenement.create(:date => "2009-07-01", :h_debut => Time.now, :h_fin => (Time.now + 4), :lieu => Faker::Lorem.words,
